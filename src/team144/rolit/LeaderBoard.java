...
public class LeaderBoard {
    ...
    public float getAverageScoreOfDay(Calendar day) {
        ...
        for (Score score : scores) {
            ...
            if (calendar.get(Calendar.YEAR) != day.get(Calendar.YEAR))
                continue;
            if (calendar.get(Calendar.MONTH) != day.get(Calendar.MONTH))
                continue;
            if (calendar.get(Calendar.DAY_OF_MONTH) != day
                    .get(Calendar.DAY_OF_MONTH)) continue;
        ...
        if (n == 0)
            throw new IllegalArgumentException("No scores exist for that day ");
        return (float) sum / n;
    }
    ...
    public List<Score> getScoresBelow(int score) {
        ...
        if (index == -1) throw new IllegalArgumentException(
                "No score lower than the specified score exists!");
